...
**계획** 탭 아래의 **포트폴리오 짜보기** 카드를 누르면 들어갑니다. 계획 탭 자체는
이미 가진 자산을 진단하는 자리이고([계획 문서](ai-review.html)), 여기 적은
**짜보기**는 아직 사지 않은 구성을 미리 짜 보는 자리입니다. **나눔터**는 짜보기의
**가져오기**에서 들어가는, 남이 짜 본 구성을 보는 자리입니다.

> **2.0에서 세그먼트를 걷어냈습니다.** 전에는 계획 안에 「살펴보기 | 짜보기」가 있고
> 그 안이 다시 「내 짜보기 | 나눔터」로 갈려 세그먼트가 3중으로 겹쳤습니다.
> 지금은 전부 눌러 들어갑니다 — 계획 → 짜보기 → 나눔터.
...
| 코드 | `v1.`로 시작하는 짧은 글자. 받는 쪽은 **가져오기 → 코드로 받기**에 붙여 넣습니다 |
...
발견하는 자리입니다. 짜보기 화면의 **가져오기 → 나눔터에서 고르기**로 들어갑니다.
같은 자리의 **코드로 받기**와 한 카드에 묶여 있습니다 — 둘 다 남의 조합을 받는 일입니다.
...
- **보기** — [전체 · 내가 올린] 세그먼트와 정렬(인기 · 최신) 드롭다운으로 고릅니다.
