...
자산 데이터를 어디에 둘지는 두 가지 중에서 고릅니다 — 로그인 없이 **둘러보기**,
또는 **앱에 직접 입력**(증권사 화면을 캡처해 채울 수 있습니다).
어느 쪽이든 자산 데이터는 사용자 기기에 있고, 앱 제공자 서버에 저장되지 않습니다.
(**Google Sheets 연동**은 1.0까지만 있었고 1.2.0에서 빠졌습니다.)
...
| 기능 | 설명 |
| --- | --- |
| [시작하기](docs/getting-started.md) | 시작 방식 · 캡처로 종목 가져오기 · 증권사 자동 채우기 |
...
| [뉴스 · 리포트](docs/news-reports.md) | 보유 종목 뉴스 · 증권사 리포트 · 투자 이벤트 |
| [구상](docs/ai-review.md) | 살펴보기(편중 진단 → 성향별 목표 제안 → 계좌별 조정안) · 짜보기(ETF 비교 · 시뮬레이션 · 구성 공유) |
| [증권사 연결](docs/broker-connection.md) | 한투·업비트·바이낸스 잔고 조회 · 연결 확인 · 계좌 자동 채우기 |
...
- iPhone / iPad / Mac (SwiftUI 공용 코드베이스) · Android (Kotlin/Compose)
- 시작하는 데 필요한 것: 없음 (둘러보기·직접 입력)
- 증권사 잔고 조회: 한국투자증권, 업비트 — 키를 넣으면 보유 종목을 자동으로 채웁니다

## 현재 상태

App Store · Mac App Store에 공개되어 있습니다(1.2.0).

- ✅ 자산 현황·순위·배분 차이·뉴스·구상
- ✅ 자산 구성 분석 (건강점수·인사이트·배당·자산 추정·목표·개선 제안)
- ✅ 두 가지 시작 방식 (둘러보기 · 직접 입력) + 캡처로 종목 가져오기
- ✅ 증권사 잔고 조회와 계좌 자동 채우기
- ✅ 기기 간 원장 공유 (Apple은 iCloud, Android는 본인 Google 드라이브)
- 🔜 1.3.0 — 짜보기(ETF 비교 · 시뮬레이션 · 구성 공유)
- 🔜 Android 일반 공개
